...
class IRNode:
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "IRNode":
        ...
        raise NotImplementedError(
            "_deepcopy_impl() must be implemented by IRNode subclasses"
        )
...
class ManagedReductionPipelineRegion(IRNode):
    ...
    Args:
        ...
        consumer_epilogue: Optional ``PyNode`` for retire-time expression replay.
    ...
    stage_count: int = 2
    tile_count: int = 1
    lifecycle: Optional[AsyncCollectiveLifecycle] = field(
        default_factory=AsyncCollectiveLifecycle
    )
    pending_tiles: List[PendingTileDescriptor] = field(default_factory=list)
    consumer_store: Optional["BufferStore"] = None
    consumer_load: Optional["BufferLoad"] = None
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "ManagedReductionPipelineRegion":
        ...
        result = ManagedReductionPipelineRegion(
            reduce_op=copy.deepcopy(self.reduce_op, memo)
            if self.reduce_op is not None
            else None,
            overlap_axis=copy.deepcopy(self.overlap_axis, memo)
            if self.overlap_axis is not None
            else None,
            stage_count=self.stage_count,
            tile_count=self.tile_count,
            lifecycle=copy.deepcopy(self.lifecycle, memo),
            pending_tiles=copy.deepcopy(self.pending_tiles, memo),
            consumer_store=copy.deepcopy(self.consumer_store, memo)
            if self.consumer_store is not None
            else None,
            consumer_load=copy.deepcopy(self.consumer_load, memo)
            if self.consumer_load is not None
            else None,
            consumer_epilogue=copy.deepcopy(self.consumer_epilogue, memo)
            if self.consumer_epilogue is not None
            else None,
            retire_target_buffer=copy.deepcopy(self.retire_target_buffer, memo)
            if self.retire_target_buffer is not None
            else None,
            retire_target_indices=copy.deepcopy(self.retire_target_indices, memo)
...
class PyNode(IRNode):
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "PyNode":
        ...
        result = PyNode(
            ...
            vars_in_scope={
                k: copy.deepcopy(v, memo) for k, v in self.vars_in_scope.items()
            },
        )
        memo[id(self)] = result
        return result
...
class BufferMatch(IRNode):
    ...
    tensor_name: Optional[
        str
    ]  # tensor name in the original code, or None if is temporary buffer
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "BufferMatch":
        if id(self) in memo:
            return memo[id(self)]
        result = BufferMatch(
            buffer=copy.deepcopy(self.buffer, memo), tensor_name=self.tensor_name
        )
        memo[id(self)] = result
        return result
...
class BufferStore(IRNode):
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "BufferStore":
        ...
        result = BufferStore(
            ...
            indices=[
                copy.deepcopy(idx, memo) if isinstance(idx, (IRNode, Axis)) else idx
...
class BufferLoad(IRNode):
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "BufferLoad":
        ...
        result = BufferLoad(
            ...
            if isinstance(self.target, IRNode)
            else self.target,
            comm=[comm._deepcopy_impl(memo) for comm in self.comm],
        )
        memo[id(self)] = result
...
class ReduceOp(IRNode):
    ...
    def _deepcopy_impl(self, memo: Dict[int, Any]) -> "ReduceOp":
        ...
        result = ReduceOp(
            ...
            src=copy.deepcopy(self.src, memo)
            if isinstance(self.src, IRNode)
            else self.src,
            comm=[comm._deepcopy_impl(memo) for comm in self.comm],
            shard_dim=copy.deepcopy(self.shard_dim, memo),
            indices=new_indices,
            managed_collective_strategy=self.managed_collective_strategy,
            async_collective_overlap_axis=copy.deepcopy(
                self.async_collective_overlap_axis, memo
            ),
            ...
            async_collective_lifecycle=copy.deepcopy(
                self.async_collective_lifecycle, memo
            ),
        )
        memo[id(self)] = result
        return result
    ...
    def __str__(self, tabs: int = 0) -> str:
        ...
        res = (
            prefix
            + f"{self.op}({self.buffer.tensor}[{_format_indices(self.indices) if self.indices is not None else None}], axes={self.axes}, src={self.src}) (collective_op={self.collective_op})"
        )
