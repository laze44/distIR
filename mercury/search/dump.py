...
def dump(program: Program):
    ...
    for axis in axes:
        ...
        if axis.parallel_info is not None:
            axis_str += (
                f", parallel_info(cards, start_dim, end_dim)={axis.parallel_info}"
            )
        print(axis_str)
    ...
    for buffer in buffers:
        print(
            f"    {buffer.tensor}: {buffer.shape}, dtype={buffer.dtype}, shard_spec={buffer.shard_spec.specs}"
        )
    ...
    for command in buffer_commands:
        if isinstance(command, BufferStore) or isinstance(command, BufferLoad):
            if len(command.comm) > 0:
                print(
                    f"    ring for {command.buffer.tensor}: {command.indices} -> {command.comm}"
                )
        else:
            if len(command.comm) > 0:
                print(
                    f"    ring for reduce{command.buffer.tensor}: {command.axes} -> {command.comm}"
                )
            elif len(command.shard_dim) > 0:
                strategy = getattr(
                    command, "managed_collective_strategy", "blocking_collective"
                )
                if strategy == "async_collective_overlap":
                    overlap_axis = getattr(
                        command, "async_collective_overlap_axis", None
                    )
                    overlap_axis_name = (
                        overlap_axis.name if overlap_axis is not None else None
                    )
                    print(
                    ...
                    print(
                        f"    all reduce for {command.buffer.tensor}: {command.axes} -> {command.shard_dim}"
                    )
    ...
    for region in pipeline_regions:
        if region.legalized:
            overlap_name = (
                region.overlap_axis.name if region.overlap_axis is not None else None
            )
            print(
                f"    legalized async pipeline for {region.reduce_op.buffer.tensor}: "
                f"overlap_axis={overlap_name}, tiles={region.tile_count}, "
                f"stages={region.stage_count}"
            )
